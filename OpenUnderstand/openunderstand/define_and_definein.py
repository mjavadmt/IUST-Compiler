...
DB_PATH = "../../database/Testing_3.oudb"
PROJECT_PATH = "../../projects/testing_legacy_code/src"
...
def traverse_parent_reach_class_or_interface(tmp_ctx):
    while type(tmp_ctx) != JavaParserLabeled.ClassDeclarationContext and type(
            tmp_ctx) != JavaParserLabeled.InterfaceDeclarationContext:
...
def form_parent(ctx, second_parent_needed):
    if second_parent_needed:
        tmp_ctx = ctx
        while type(tmp_ctx) != JavaParserLabeled.InterfaceMethodDeclarationContext and type(tmp_ctx) != \
                JavaParserLabeled.MethodDeclarationContext and type(
            tmp_ctx) != JavaParserLabeled.ConstructorDeclarationContext:
            tmp_ctx = tmp_ctx.parentCtx
        if type(tmp_ctx) != JavaParserLabeled.ConstructorDeclarationContext:
            parent_name_1 = tmp_ctx.children[
                1].getText()  # parent name which is the method name this parameter declared in
        else:
            parent_name_1 = tmp_ctx.children[
                0].getText()  # parent name which is the method name this parameter declared in
...
def add_declaration(ctx, which_declaration, kind_name, save_dict, prefix_needed, second_parent_needed, stop_prefix,
                    is_generic, class_or_interface, needs_content, value_needed):
    entity_type, entity_name, entity_value = derive_type_and_name(which_declaration, ctx, value_needed)

    parent_name = form_parent(ctx, second_parent_needed) if not class_or_interface else ""

    line = ctx.start.line  # get line
    column = ctx.start.column  # get column

    save_dict[f"{parent_name}.{entity_name}.{line}"] = {
        "name": entity_name,
        "type": entity_type,
        "value": entity_value,
    ...
    if prefix_needed[0]:
        save_dict[f"{parent_name}.{entity_name}.{line}"]["prefixes"] = form_prefix(ctx, prefix_needed, stop_prefix)
    else:
        save_dict[f"{parent_name}.{entity_name}.{line}"]["kind_name"] = kind_name

    if is_generic:
        save_dict[f"{parent_name}.{entity_name}.{line}"]["prefixes"] += " generic"


def define_is_generic(ctx):
    child_types = list(map(lambda x: type(x), ctx.children))
    is_generic = True if JavaParserLabeled.TypeParametersContext in child_types else False
    return is_generic
...
class DefineListener(JavaParserLabeledListener):
    ...
    def enterPackageDeclaration(self, ctx: JavaParserLabeled.PackageDeclarationContext):
        self.package["package_name"] = ctx.getText().replace("package", "").replace(";", "")
        self.package["line"] = ctx.start.line
        self.package["column"] = ctx.start.column

    def enterFormalParameter(self, ctx: JavaParserLabeled.FormalParameterContext):
        add_declaration(prefix_needed=(False, 0), ctx=ctx, which_declaration=1, kind_name="Java Parameter",
                        save_dict=self.formal_parameters,
                        second_parent_needed=True, stop_prefix=False, is_generic=False, class_or_interface=False,
                        needs_content=False, value_needed=False)

    def enterInterfaceMemberDeclaration0(self, ctx: JavaParserLabeled.InterfaceMemberDeclaration0Context):
        add_declaration(prefix_needed=(True, 1), ctx=ctx, which_declaration=2, kind_name="",
                        save_dict=self.fields,
                        second_parent_needed=False, stop_prefix=JavaParserLabeled.InterfaceMemberDeclaration0Context,
                        is_generic=False, class_or_interface=False, needs_content=False, value_needed=True)

    def enterFieldDeclaration(self, ctx: JavaParserLabeled.FieldDeclarationContext):
        add_declaration(prefix_needed=(True, 2), ctx=ctx, which_declaration=3, kind_name="",
                        save_dict=self.fields,
                        second_parent_needed=False, stop_prefix=JavaParserLabeled.MemberDeclaration2Context,
                        is_generic=False, class_or_interface=False, needs_content=False, value_needed=True)
    ...
    def enterLocalVariableDeclaration(self, ctx: JavaParserLabeled.LocalVariableDeclarationContext):
        ...
        if ctx.children[0].getText() == "final":
            is_final = "Final "
        add_declaration(prefix_needed=(False, 0), ctx=ctx, which_declaration=5,
                        kind_name=f"Java {is_final}Variable Local",
                        save_dict=self.local_variables,
                        second_parent_needed=True, stop_prefix=False, is_generic=False, class_or_interface=False,
                        needs_content=False, value_needed=True)

    def enterInterfaceMethodDeclaration(self, ctx: JavaParserLabeled.InterfaceMethodDeclarationContext):
        add_declaration(prefix_needed=(True, 2), ctx=ctx, which_declaration=1, kind_name="",
                        save_dict=self.methods,
                        second_parent_needed=False, stop_prefix=JavaParserLabeled.InterfaceMemberDeclaration1Context,
                        is_generic=False, class_or_interface=False, needs_content=True, value_needed=False)

    def enterMethodDeclaration(self, ctx: JavaParserLabeled.MethodDeclarationContext):
        add_declaration(prefix_needed=(True, 2), ctx=ctx, which_declaration=1, kind_name="",
                        save_dict=self.methods,
                        second_parent_needed=False, stop_prefix=JavaParserLabeled.MemberDeclaration0Context,
                        is_generic=False, class_or_interface=False, needs_content=True, value_needed=False)

    def enterConstructorDeclaration(self, ctx: JavaParserLabeled.ConstructorDeclarationContext):
        add_declaration(prefix_needed=(True, 2), ctx=ctx, which_declaration=6, kind_name="",
                        save_dict=self.methods,
                        second_parent_needed=False, stop_prefix=JavaParserLabeled.MemberDeclaration3Context,
                        is_generic=False, class_or_interface=False, needs_content=True, value_needed=False)

    def enterInterfaceDeclaration(self, ctx: JavaParserLabeled.InterfaceDeclarationContext):
        add_declaration(prefix_needed=(True, 1), ctx=ctx, which_declaration=4, kind_name="",
                        save_dict=self.interfaces,
                        second_parent_needed=False, stop_prefix=JavaParserLabeled.InterfaceDeclarationContext,
                        is_generic=define_is_generic(ctx), class_or_interface=True, needs_content=True,
                        value_needed=False)

    def enterClassDeclaration(self, ctx: JavaParserLabeled.ClassDeclarationContext):
        add_declaration(prefix_needed=(True, 1), ctx=ctx, which_declaration=4, kind_name="",
                        save_dict=self.classes,
                        second_parent_needed=False, stop_prefix=JavaParserLabeled.ClassDeclarationContext,
                        is_generic=define_is_generic(ctx), class_or_interface=True, needs_content=True,
                        value_needed=False)
...
def add_defined_entities(entities, entity_type, package_name, file_path):
    for entity_key, entity_values in entities.items():
        ...
        if entity_type == "method" and entity_values["type"] == "":
            is_constructor = True
        kind_str = entity_values[
            "kind_name"] if entity_type == "local variable" or entity_type == "parameter" else extract_all_kind(
...
def main():
    ...
    for file_path, file_name in zip(p.file_paths, p.file_names):
        try:
            ...
            add_defined_entities(define_listener.classes, "class", package_name, file_path)
            add_defined_entities(define_listener.interfaces, "interface", package_name, file_path)
            add_defined_entities(define_listener.fields, "variable", package_name, file_path)
